...
class StationsService {
  ...
  static _checkIfInitialized() async {
    ...
    if (!_initialized) {
      ...
      setTimeout(secondsToNextUpdate, () {
  ...
  static _fetchData() async {
    ...
    try {
      ...
      for (var station in _stationCodes) {
        ...
        var lastData = deSerializedData[deSerializedData.length - 1]; // Latest data point

        // If data for station is valid add it to stations list
        if (lastData.length == 2 && lastData[1] is double) {
          _stations.add(Station(station.name, StationsService.setStation, activity: lastData[1]));
        }
